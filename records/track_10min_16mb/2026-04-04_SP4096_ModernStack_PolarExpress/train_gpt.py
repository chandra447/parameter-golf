...
class Hyperparameters():
    ...
    nope_every_n = int(os.environ.get('NOPE_EVERY_N', 0))  # disabled: conflicts with recurrence L4 and parallel residuals L7+
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int,
        ...
        self.use_xsa = False
        self.use_rope = True  # NoPE: set False to skip RoPE for this layer
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        self.parallel = parallel  # ParallelResiduals: compute attn+mlp from same input

    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        if self.parallel:
            # Parallel residuals: attn and mlp both read from x_in (same pre-norm input)
...
class GPT(nn.Module):
    def __init__(self, h: Hyperparameters):
        ...
        # NoPE: every nth layer skips RoPE (enables global attention without positional bias)
        if h.nope_every_n > 0:
            for i, block in enumerate(self.blocks):
                if i % h.nope_every_n == 0:
                    block.attn.use_rope = False
        # Parallel residuals for layers >= parallel_start_layer
        if h.parallel_start_layer >= 0:
...
def zeropower_via_newtonschulz5(G: Tensor, steps: int = 10, eps: float = 1e-7) -> Tensor:
    # Polar Express: 4-step minimax coefficients (arXiv:2505.16932)
    # steps param kept for API compat; we always run exactly 4 steps
    _POLAR_STEPS = [
    ...
    if transposed:
        X = X.T
    for a, b, c in _POLAR_STEPS:
        A = X @ X.T
        B = b * A + (c * A @ A if c != 0.0 else torch.zeros_like(A))
        X = a * X + B @ X
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(params):
                if i % world_size == rank and p.grad is not None:
                    ...
                    # MuonEq-R: row-normalize gradient before Newton-Schulz (arXiv:2603.28254)
                    if g.ndim == 2:
...
def train_model(h: Hyperparameters, device: torch.device, val_data: ValidationData) -> None:
    ...
    recur_layer_pairs = []
    if h.recur_layers:
        parts = [int(x.strip()) for x in h.recur_layers.split(',') if x.strip()]
        if len(parts) >= 2:
            for j in range(0, len(parts) - 1, 2):
                recur_layer_pairs.append((parts[j], parts[j+1]))
    ...
    _TRACKIO = False
    try:
        import trackio as _trackio_mod
        _TRACKIO = True
    except ImportError:
        pass
    if _TRACKIO and h.is_main_process:
        _trackio_mod.init(
            project="parameter-golf",
            name=os.environ.get("TRACKIO_RUN_NAME", f"sp4096_modern_nope_seed{h.seed}"),
            space_id=os.environ.get("TRACKIO_SPACE_ID", "Chandra447/trackio"),
            config={
                "vocab_size": h.vocab_size,
                ...
                "recur_layers": h.recur_layers,
                "recur_start_step": h.recur_start_step,
                "parallel_start_layer": h.parallel_start_layer,
                "nope_every_n": h.nope_every_n,
                "seed": h.seed,
            },
        )
    ...
    _recurrence_applied = [False]

    def _maybe_apply_recurrence(step: int) -> None:
        if _recurrence_applied[0]:
            return
        if step >= h.recur_start_step and recur_layer_pairs:
            log(f"depth_recurrence:activating at step={step} pairs={recur_layer_pairs}")
            for src_idx, tgt_idx in recur_layer_pairs:
                if src_idx < len(base_model.blocks) and tgt_idx < len(base_model.blocks):
                    # Share MLP weights: tgt's MLP becomes the same nn.Module as src's
                    base_model.blocks[tgt_idx].mlp = base_model.blocks[src_idx].mlp
                    log(f"depth_recurrence:blocks[{tgt_idx}].mlp <- blocks[{src_idx}].mlp (shared)")
            _recurrence_applied[0] = True

    def step_fn(step, lr_scale):
        _maybe_apply_recurrence(step)
    ...
    while True:
        ...
        if should_validate:
            ...
            if _TRACKIO and h.is_main_process:
                _trackio_mod.log({"val/bpb": val_bpb, "val/loss": val_loss, "val/step": step})
            torch.cuda.synchronize()
            t0 = time.perf_counter()
        ...
        if should_log_train:
            ...
            if _TRACKIO and h.is_main_process:
                _trackio_mod.log({
                    ...
                    "train/bpb": train_loss.item() / math.log(2),
                    "train/step": step,
